...
def imm_process(input_data, output_data, spark):
    '''
    This function contains all the functions and manipulations needed to shape and write the finished immigration table
    ...
    #read in the other file(s) with different schema shape
    ...
    columns = ['cicid', 'i94mon', 'i94res','i94port','arrdate', 'i94mode','i94addr',
            'depdate','i94bir', 'i94visa', 'dtadfile', 'biryear', 'dtaddto',
    ...
    imm_df = imm_df.dropDuplicates(['cicid'])\
    ...
    imm_df = imm_df.withColumn('i94_month', imm_df.i94_month.cast(IntegerType()))\
                   .withColumn('age', imm_df.age.cast(IntegerType()))\
                   .withColumn('birth_year', imm_df.birth_year.cast(IntegerType()))
    ...
    #display data as quality check
    imm_df.count()
    imm_df.show(5)

    #write the table to parquet file
    #imm_df.write.partitionBy('i94_month').mode('overwrite').parquet(os.path.join(output_data, 'immigration/''))
    imm_df.write.mode('overwrite').parquet(os.path.join(output_data, 'immigration/'))
