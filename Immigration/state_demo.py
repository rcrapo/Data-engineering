...
def demo_process(input_data, output_data, spark):
    ...
    state_demo = state_demo.select('state_code', 'native_american',
                                    'asian', 'aa_black', 'hispanic_latino',
                                    'white',  'num_of_veterans', 'foreign_born',
                                    'male_population', 'female_population',
                                    'total_population', 'median_age', 'avg_household_size')


    #print final table info as a quality check
    state_demo.count()
    state_demo.show(5)
